...
const createReview = async function (req, res) {
  try {
    ...
    if (!isValidRequestBody(req.body)) {
      return res
        .status(400)
        .send({ status: false, message: "Please enter required parameters" });
    }
...
const deleteReview = async function (req, res) {
  try {
    ...
    const validBook = await bookModel.findOne({
      _id: bookId,
      isDeleted: false,
    });
    if (!validBook) {
    ...
    return res
      ...
      .send({
        status: true,
        message: "Review deleted and book's review updated successfully",
      });
  } catch (err) {
    return res.status(500).send({ status: false, message: err.message });
  }
